...
def mail_gen():
    ...
    no_of_random_mails=input("Berapa banyak mail yang Anda butuhkan : ")    
...
def mailbox_msgs():
    ...
    if str(mailbox_json)=='[]':
        ...
        for i in range(no_of_msgs):
                  ...
                  print('Nomor Mail : '+str(id))
                  print('Mail Dari : '+sender)
                  print('Waktu Diterima : '+maildate)
...
def dl_attachments():
      ...
      if att_mailboxapiurl=='https://www.1secmail.com/api/v1/?action=download&login='+mailid+'&domain='+domain+'&id='+id+'&file=':
        ...
        print('\nLampiran Berhasil Disimpan sebagai '+filename+'\n')
  ...
  if checkmethod=='1':
    mail_gen()
      ...
      Pilih Domain
       ...
       ''')
      select_domain=str(input('Pilih Opsi : '))
      if select_domain=='1':
            domain='1secmail.com'
        ...
        domain='wuuvo.com'
          ...
          banner()
          main()
      enter_user=input('Masukkan nama pengguna : ')
      mailid=enter_user
      print('Mail Kamu Adalah '+mailid+'@'+domain)
      for i in range(100):
          input('refresh inbox tekan enter, untuk stop ctrl+c')
          mailbox_msgs()
          if no_of_msgs>0:
            check_show=input('Masukkan Nomor Mail Untuk Mengakses Pesan : ')
            id=check_show
            read_mailboxmgs()
            dl_attachments()
          else:
    ...
    print('WARNING : Silakan Pilih Opsi yang Benar [1/2] ')
    main()
